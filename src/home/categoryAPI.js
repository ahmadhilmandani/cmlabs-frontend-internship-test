      ...
      $.map(foodListCategory, function (value, index) {
        ...
        axios.get('https://www.themealdb.com/api/json/v1/1/filter.php?c=' + value.strCategory)
          .then(function (response) {
            return $('main').append(
              `
              <a href=../foodListByCategory/foodListByCategory.html?c=${value.strCategory}>
                <div class="bg-white rounded-2xl p-3 w-60 min-h-72 shadow-md relative z-10 group hover:-translate-y-3 transition-all duration-500">
                  <div class="bg-[#FFF9F8] rounded-lg w-full aspect-square overflow-hidden bg-cover bg-center cursor-pointer"
